...
bool
IdCache::IsDuplicateWithMetric(Ipv4Address addr,
                               uint32_t id,
                               double metric)
{
    Purge();
    ...
    for (auto i = m_idCache.begin(); i != m_idCache.end(); ++i)
    {
        if (i->m_context == addr && i->m_id == id)
        {
            // Entry already exists → compare metric

            if (metric < i->m_metric)
            {
                // Better metric found → update stored metric
                i->m_metric = metric;
                i->m_expire = m_lifetime + Simulator::Now();
                return false;  // Not duplicate (better path)
            }
            else
            {
                return true;   // Duplicate (worse or equal)
            }
        }
    }
    ...
    UniqueId uniqueId;
    uniqueId.m_context = addr;
    uniqueId.m_id = id;
    uniqueId.m_expire = m_lifetime + Simulator::Now();
    uniqueId.m_metric = metric;
