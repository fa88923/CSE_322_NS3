...
/**
 * Set custom routing metric
 */
void RoutingTableEntry::SetMetric(double metric)
{
    m_metric = metric;
}
 ...
 */
double RoutingTableEntry::GetMetric() const
{
    return m_metric;
}
...
RoutingTableEntry::Invalidate(Time badLinkLifetime)
{
    ...
    m_metric = std::numeric_limits<double>::infinity();
}
...
RoutingTable::AddRoute(RoutingTableEntry& rt)
{
    ...
    if (result.second)
    {
        NS_LOG_LOGIC("ROUTE CREATED: Dst=" << rt.GetDestination() 
                     << " | NextHop=" << rt.GetNextHop() 
                     << " | Hops=" << (uint16_t)rt.GetHop() 
                     << " | Metric=" << rt.GetMetric() 
                     << " | SeqNo=" << rt.GetSeqNo() 
                     << " | Lifetime=" << rt.GetLifeTime().GetSeconds() << "s"
                     << " | Flag=" << (rt.GetFlag() == VALID ? "VALID" : (rt.GetFlag() == INVALID ? "INVALID" : "IN_SEARCH")));
    }
...
RoutingTable::Update(RoutingTableEntry& rt)
{
    ...
    NS_LOG_LOGIC("ROUTE UPDATE: Dst=" << rt.GetDestination() 
                 << " | OldMetric=" << i->second.GetMetric() 
                 << " | NewMetric=" << rt.GetMetric() 
                 << " | OldHops=" << (uint16_t)i->second.GetHop() 
                 << " | NewHops=" << (uint16_t)rt.GetHop() 
                 << " | OldNextHop=" << i->second.GetNextHop() 
                 << " | NewNextHop=" << rt.GetNextHop() 
                 << " | NewSeqNo=" << rt.GetSeqNo() 
                 << " | NewLifetime=" << rt.GetLifeTime().GetSeconds() << "s");
