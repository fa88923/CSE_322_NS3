...
class RoutingProtocol : public Ipv4RoutingProtocol
{
  ...
  private:
    ...
    void SetIacpEnable(bool enable)
    {
        m_enableIacp = enable;
    }
  ...
  // ---------------- IACR INFORMATION COLLECTION ----------------
    ...
    // Per-node information table (neighbor -> interference info)
    std::map<Ipv4Address, IcpEntry> m_icpTable;
    std::map<Ipv4Address, IcpEntryOWN> m_icpTableOWN;

    double GetIcpMetric(double created_interference, double received_interference, double hopCount, double delta = 0.8);
    double GetMetricSelf(Ipv4Address sender, double totalCreatedInterferenceBySender, double hopCount);
    double GetMetricNeighbour(Ipv4Address neighbor, double hopCount);



    double GetMinNeighborMetric(Ipv4Address& bestNeighbor);

    // // ---------------- END OF IACR INFORMATION COLLECTION ----------------

    // // Compute interference metrics for a neighbor
    // double ComputeCreatedInterference(Ipv4Address neighbor);
    // double ComputeReceivedInterference();
    // double ComputeAggregateInterference(double Icj, double Ij);

    // // Update ICP entry for a neighbor
    // void UpdateIcpEntry(Ipv4Address neighbor,
    //                     double Icj,
    //                     double Ij,
    //                     double Iaggr);

    // // Process received ICP info from neighbor
    // void ProcessIcpInfo(Ipv4Address neighbor,
    //                     double Icj,
    //                     double Ij,
    //                     double Iaggr);
    ...
    void SendIcReply(Ipv4Address destination,Ipv4Address source,double receivedPower, double receivedInterference);
    void RecvIcReply(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender);
    // At the end of private or public section
    TracedCallback<Ipv4Address, Ipv4Address, double, double> m_icReplyTrace;
    /*
