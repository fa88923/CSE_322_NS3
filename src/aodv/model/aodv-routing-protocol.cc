 ...
 */
#define NS_LOG_APPEND_CONTEXT  // Disabled to avoid template expansion issues
...
RoutingProtocol::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::aodv::RoutingProtocol")
            ...
            .AddTraceSource("IcRepSent",
                            "Trace source fired when an IC-REP packet is sent",
                            MakeTraceSourceAccessor(&RoutingProtocol::m_icRepSentTrace),
                            "ns3::aodv::RoutingProtocol::IcRepTracedCallback")
            .AddTraceSource("IcRepRecv",
                            "Trace source fired when an IC-REP packet is received",
                            MakeTraceSourceAccessor(&RoutingProtocol::m_icRepRecvTrace),
                            "ns3::aodv::RoutingProtocol::IcRepTracedCallback")
            .AddTraceSource("IcpEntryUpdate",
                            "Trace source fired when an ICP entry is updated",
                            MakeTraceSourceAccessor(&RoutingProtocol::m_icpEntryUpdateTrace),
                            "ns3::aodv::RoutingProtocol::IcpEntryTracedCallback");
    return tid;
}
...
RoutingProtocol::NotifyInterfaceUp(uint32_t i)
{
    ...
    if (wifi)
    {
        ...
        phy->TraceConnectWithoutContext(
            "MonitorSnifferRx",
            MakeCallback(&RoutingProtocol::PhyRxEndTrace, this));
        
        //Set thermal noise for the interface
        // Standard WiFi noise figure is typically 7 dB
        double noiseFigureDb = 7.0;
        double k = 1.38064852e-23; // Boltzmann
        double T = 290;           // Kelvin
        double B = 20e6;          // 20 MHz typical WiFi channel

        double thermalNoise = k * T * B;
...
// double
// RoutingProtocol::GetMinNeighborMetric(Ipv4Address& bestNeighbor)
// {
//     double delta = 0.5;
//     double minMetric = std::numeric_limits<double>::infinity();
//     bestNeighbor = Ipv4Address();

//     for (const auto& entry : m_icpTable)
//     {
//         Ipv4Address neighbor = entry.first;
//         double created_interference = entry.second.createdInterference;
//         double received_interference = entry.second.receivedInterference;
//         double metric = delta * created_interference + (1 - delta) * received_interference;
...
//     if (minMetric == std::numeric_limits<double>::infinity())
//     {
//         minMetric = 0.0;
//     }
...
double RoutingProtocol::getTotalCreatedInterference()
{
    double total = 0.0;
    for (const auto& entry : m_icpTable)
    {
...
RoutingProtocol::SendRequest(Ipv4Address dst)
{
    ...
    // // Initialize RREQ metric at source
    // rreqHeader.SetMetric(minMetric);
...
RoutingProtocol::RecvAodv(Ptr<Socket> socket)
{
    ...
    NS_LOG_DEBUG("About to check TypeHeader validity. tHeader.Get()=" << (uint32_t)tHeader.Get() 
                 << ", packet size after RemoveHeader=" << packet->GetSize());
    ...
    NS_LOG_DEBUG("AODV message received with type: " << (uint32_t)tHeader.Get() 
                 << " (ICREP=" << AODVTYPE_ICREP << ", RREQ=" << AODVTYPE_RREQ 
                 << ", RREP=" << AODVTYPE_RREP << ")");
    switch (tHeader.Get())
    {
    case AODVTYPE_RREQ: {
    ...
    case AODVTYPE_ICREP: {
        RecvIcReply(packet, receiver, sender);
        break;
    }
    }
}
...
double RoutingProtocol::GetMetricSelf(Ipv4Address sender, double totalCreatedInterferenceBySender)
{
    ...
    double metric = 0;
    auto itNeighbor = m_icpTableOWN.find(sender);
    if (itNeighbor != m_icpTableOWN.end())
    {
...
double RoutingProtocol::GetMetricNeighbour(Ipv4Address neighbor)
{
    ...
    double receivedInterference = 0.0;
    double metric = 0;
    auto itNeighbor = m_icpTable.find(neighbor);
    if (itNeighbor != m_icpTable.end())
    {
...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    //bool hasPrevRoute = m_routingTable.LookupRoute(src, toPrev);
    ...
    if (m_rreqIdCache.IsDuplicateWithMetric(origin, id, metric))
    {
    ...
    // double pathMetric = rreqHeader.GetMetric();
    // pathMetric += minMetric;

    // rreqHeader.SetMetric(pathMetric);
    ...
    if (!m_routingTable.LookupRoute(origin, toOrigin))
    {
        ...
        newEntry.SetMetric(prevMetric);
        m_routingTable.AddRoute(newEntry);
    }

    //Original update of the origin route
    // else
    // {
    ...
    else
    {
        ...
        // Case 1: No valid seqno OR newer seqno → update
        if (!toOrigin.GetValidSeqNo() ||
            int32_t(rreqHeader.GetOriginSeqno()) - int32_t(toOrigin.GetSeqNo()) > 0)
        {
            shouldUpdate = true;
        }

        // Case 2: Same seqno → compare metric
        else if (rreqHeader.GetOriginSeqno() == toOrigin.GetSeqNo())
        {
            if (prevMetric < toOrigin.GetMetric())
            {
                shouldUpdate = true;
            }
        }
        ...
        if (shouldUpdate)
        {
            ...
            toOrigin.SetHop(hop);              // keep for compatibility
            toOrigin.SetMetric(prevMetric);    // 
    ...
    if (!m_routingTable.LookupRoute(src, toNeighbor))
    {
        ...
        newEntry.SetMetric(LinkCostToPrev);
        m_routingTable.AddRoute(newEntry);
    }
    else      //most recent info diye store
    {
        ...
        toNeighbor.SetHop(1);
        toNeighbor.SetMetric(LinkCostToPrev);     // Might not necessarily be the best metric, but at least it's the most recent info we have about the neighbor
                                             ...
                                             bool gratRep)
{
    ...
    rrepHeader.SetPrevMetric(toDst.GetMetric());
    ...
    if (gratRep)
    {
        ...
        gratRepHeader.SetPrevMetric(toOrigin.GetMetric());
...
RoutingProtocol::RecvReply(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
    ...
    if (m_routingTable.LookupRoute(dst, toDst))
    {
        ...
        if (
            ...
            (rrepHeader.GetDstSeqno() == toDst.GetSeqNo() && metric < toDst.GetMetric()) ||
...
RoutingProtocol::ProcessHello(const RrepHeader& rrepHeader, Ipv4Address receiver)
{
    ...
    else if(toNeighbor.GetNextHop() == rrepHeader.GetDst())     //if conditionta notun kore add korsi
    {
        ...
        //toNeighbor.SetMetric(rrepHeader.GetPrevMetric());
        m_routingTable.Update(toNeighbor);
    }
    if (m_enableHello)
    {
    ...
    double receivedPower = m_lastRxSignalW;
    double receivedInterference = m_lastInterferenceW;

    IcpEntryOWN entry;
    entry.neighbor = helloSender;  
    entry.createdInterference = receivedPower;  // I_c^j
    entry.receivedInterference = receivedInterference;         // I_j

    entry.lastUpdate = Simulator::Now();
    
    m_icpTableOWN[helloSender] = entry;

    NS_LOG_DEBUG("HELLO from " << helloSender 
                << " - ReceivedPower=" << receivedPower 
                << "W, ReceivedInterference=" << receivedInterference << "W");

    SendIcReply(helloSender, receiver, receivedPower, receivedInterference);
}


void
RoutingProtocol::PhyRxEndTrace(Ptr<const Packet> packet,
                               uint16_t frequency,
                               ...
                               uint16_t staId)
{
    double signalW = std::pow(10.0, signalNoise.signal / 10.0) / 1000.0;
    double totalNoiseW = std::pow(10.0, signalNoise.noise / 10.0) / 1000.0;

    double interferenceW = totalNoiseW - m_thermalNoiseW;
    if (interferenceW < 0)
        interferenceW = 0;

    m_lastRxSignalW = signalW;
    m_lastInterferenceW = interferenceW;
}

void
RoutingProtocol::SendIcReply(Ipv4Address destination,
                              Ipv4Address source,
                              double receivedPower,
                              double receivedInterference)
{
    NS_LOG_FUNCTION(this << destination << source);
    NS_LOG_DEBUG("SendIcReply: Preparing to send IC-REP from " << source << " to " << destination
                 << " with RxPower=" << receivedPower << "W, RxInterference=" << receivedInterference << "W");
    

    IcRepHeader icRepHeader(source,              // transmitter 
                            receivedPower,       // power received from destination
                            receivedInterference); // total interference received
    

    NS_LOG_DEBUG("IC-REP header created: Transmitter=" << icRepHeader.GetTransmitter()
                 << ", RxPower=" << icRepHeader.GetReceivedPower()
                 << "W, RxInterference=" << icRepHeader.GetInterference() << "W");
    
                 
    Ptr<Packet> packet = Create<Packet>();
    packet->AddHeader(icRepHeader);
    
    
    TypeHeader tHeader(AODVTYPE_ICREP);
    packet->AddHeader(tHeader);
    ...
    NS_LOG_DEBUG("SendIcReply: IC-REP packet created, size=" << packet->GetSize() << " bytes, TTL=1");
    
    
    Ptr<Socket> socket = FindSocketWithInterfaceAddress(
        m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(source), 0)
    );
    
    if (!socket)
    {
        NS_LOG_ERROR("No socket found for interface " << source);
        return;
    }
    
    NS_LOG_DEBUG("SendIcReply: Socket found, sending to " << destination << ":" << AODV_PORT);
    
    
    int result = socket->SendTo(packet, 0, InetSocketAddress(destination, AODV_PORT));
    
    NS_LOG_DEBUG("SendIcReply: SendTo returned " << result);
    
    if (result < 0)
    {
        NS_LOG_ERROR("Failed to send IC-Reply to " << destination 
                     << " from interface " << source 
                     << " (SendTo returned " << result << ")");
        return;
    }
    
    NS_LOG_DEBUG("SendIcReply: IC-REP packet successfully sent. Firing trace...");
    
    m_icRepSentTrace(source, destination, receivedPower, receivedInterference);
    
    NS_LOG_DEBUG("IC-Reply sent to " << destination 
                 << " with ReceivedPower=" << receivedPower 
                 << "W, ReceivedInterference=" << receivedInterference << "W");
}


void
RoutingProtocol::RecvIcReply(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
    NS_LOG_FUNCTION(this << receiver << sender);
    
    IcRepHeader icRepHeader;
    p->RemoveHeader(icRepHeader);
    
    Ipv4Address transmitter = icRepHeader.GetTransmitter();
    
    if (transmitter != sender)
    {
        NS_LOG_DEBUG("IC-Reply transmitter mismatch. Header: " << transmitter 
                     << " IP sender: " << sender << ". Using IP sender as source.");
    }
    
    double receivedPower = icRepHeader.GetReceivedPower();
    double receivedInterference = icRepHeader.GetInterference();
    
    NS_LOG_DEBUG("IC-Reply header values - ReceivedPower: " << receivedPower 
                 << "W, ReceivedInterference: " << receivedInterference << "W");
    
    double createdInterferenceAtSender = receivedPower;
    
    NS_LOG_DEBUG("IC-Reply from " << sender 
                 << " - We create " << createdInterferenceAtSender 
                 << "W at " << sender 
                 << ", " << sender << " receives " << receivedInterference << "W total");
    

    IcpEntry entry;
    entry.neighbor = sender; 
    entry.createdInterference = createdInterferenceAtSender;  // I_c^j
    entry.receivedInterference = receivedInterference;         // I_j

    entry.lastUpdate = Simulator::Now();
    
    m_icpTable[sender] = entry;
    
    m_icRepRecvTrace(sender, receiver, receivedPower, receivedInterference);
    m_icpEntryUpdateTrace(receiver, sender, entry.createdInterference, entry.receivedInterference, entry.aggregateInterference);
    
    NS_LOG_INFO("ICP Table updated for neighbor " << sender 
...
RoutingProtocol::SendHello()
{
    ...
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        ...
        // IcReqHeader icReq;
        // icReq.SetTransmitter(iface.GetLocal());
        // // icReq.SetTxPower(GetMaxTxPower());   // must be max power
        // packet->AddHeader(icReq);
