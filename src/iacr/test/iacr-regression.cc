 ...
 * @ingroup iacr-test
 *
 * @brief IACR regression test suite
 */
class IacrRegressionTestSuite : public TestSuite
{
  public:
    IacrRegressionTestSuite()
        : TestSuite("routing-iacr-regression", Type::SYSTEM)
    {
        SetDataDir(NS_TEST_SOURCEDIR);
        // General RREQ-RREP-RRER test case
        AddTestCase(new ChainRegressionTest("iacr-chain-regression-test"),
...
} g_iacrRegressionTestSuite; ///< the test suite
...
ChainRegressionTest::ChainRegressionTest(const char* const prefix,
    ...
    : TestCase("IACR chain regression test"),
      m_nodes(nullptr),
      m_prefix(prefix),
      m_time(t),
      m_size(size),
...
ChainRegressionTest::CreateDevices()
{
    ...
    // 2. Setup TCP/IP & IACR
    IacrHelper iacr; // Use default parameters here
    InternetStackHelper internetStack;
    internetStack.SetRoutingHelper(iacr);
    ...
    streamsUsed += iacr.AssignStreams(*m_nodes, streamsUsed);
    // IACR uses m_size more streams
