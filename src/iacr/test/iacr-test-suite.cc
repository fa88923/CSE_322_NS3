 ...
 */
#include "ns3/iacr-neighbor.h"
#include "ns3/iacr-packet.h"
#include "ns3/iacr-rqueue.h"
#include "ns3/iacr-rtable.h"
...
{
    TypeHeaderTest()
        : TestCase("IACR TypeHeader")
    {
    }

    void DoRun() override
    {
        TypeHeader h(IACRTYPE_RREQ);
        NS_TEST_EXPECT_MSG_EQ(h.IsValid(), true, "Default header is valid");
        NS_TEST_EXPECT_MSG_EQ(h.Get(), IACRTYPE_RREQ, "Default header is RREQ");

        Ptr<Packet> p = Create<Packet>();
        p->AddHeader(h);
        TypeHeader h2(IACRTYPE_RREP);
 ...
 * @ingroup iacr-test
 *
 * @brief Unit test for IACR routing table entry
 */
...
struct IacrRqueueTest : public TestCase
{
    IacrRqueueTest()
        : TestCase("Rqueue"),
          q(64, Seconds(30))
...
IacrRqueueTest::DoRun()
{
    ...
    h.SetDestination(Ipv4Address("1.2.3.4"));
    h.SetSource(Ipv4Address("4.3.2.1"));
    Ipv4RoutingProtocol::UnicastForwardCallback ucb = MakeCallback(&IacrRqueueTest::Unicast, this);
    Ipv4RoutingProtocol::ErrorCallback ecb = MakeCallback(&IacrRqueueTest::Error, this);
    ...
    Simulator::Schedule(q.GetQueueTimeout() + Seconds(1), &IacrRqueueTest::CheckTimeout, this);
 ...
 * @brief Unit test for IACR routing table entry
 */
struct IacrRtableEntryTest : public TestCase
{
    IacrRtableEntryTest()
 ...
 * @ingroup iacr-test
 *
 * @brief Unit test for IACR routing table
 */
struct IacrRtableTest : public TestCase
{
    IacrRtableTest()
        : TestCase("Rtable")
    {
 ...
 * @ingroup iacr-test
 *
 * @brief IACR test suite
 */
class IacrTestSuite : public TestSuite
{
  public:
    IacrTestSuite()
        : TestSuite("routing-iacr", Type::UNIT)
    {
        ...
        AddTestCase(new IacrRqueueTest, TestCase::Duration::QUICK);
        AddTestCase(new IacrRtableEntryTest, TestCase::Duration::QUICK);
        AddTestCase(new IacrRtableTest, TestCase::Duration::QUICK);
    }
} g_iacrTestSuite; ///< the test suite
